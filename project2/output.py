...
def render_product_pair_advisor():
    ...
    if not filtered.empty:
        for _, row in filtered.iterrows():
            # Adding the box with white background and the text styling
            st.markdown(f"""
            <div style="border: 2px solid #ddd; border-radius: 10px; padding: 20px; margin: 10px 0; background-color: white;">
                <p style="font-weight: bold; color: #2c3e50;">🛒 <span style="color: #16a085;">Bought:</span> {row['antecedents']}</p>
                <p style="font-weight: bold; color: #2c3e50;">➕ <span style="color: #e74c3c;">Recommend:</span> {row['consequents']}</p>
                <p style="font-weight: bold; color: #2c3e50;">🎯 <span style="color: #3498db;">Success Rate:</span> {row['success_rate']}%</p>
                <p style="font-weight: bold; color: #2c3e50;">📊 <span style="color: #9b59b6;">Frequency:</span> {row['combination_frequency']}%</p>
            </div>
